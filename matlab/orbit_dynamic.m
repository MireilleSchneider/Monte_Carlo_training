...
disp('------------------------------------------------------')
disp('You may want to modify norb, i.e. the number of points')
disp('to take into account for initial and final orbits')
disp(['Right now: norb = ',num2str(norb)])
disp('------------------------------------------------------')
npt    = length(rpa_orb);
nstart = min(npt,norb);
nend   = max(1,npt-norb);
...
plot(rpa_orb(1:nstart),zpa_orb(1:nstart),'r','linewidth',linewidth-1)
plot(rpa_orb(nend:end),zpa_orb(nend:end),'k','linewidth',linewidth-1)

print -dpng ../fig/orbit_dynamic.png
print -depsc ../fig/orbit_dynamic.eps
