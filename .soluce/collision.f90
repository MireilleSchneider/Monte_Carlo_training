subroutine collision(scal,inp1d,inp2d,bigstep,particle,out1d)
  ...
  if(scal%xiflag.eq.1) then ! FLAG TELLING IF THIS PROCESS MUST BE CONSIDERED
     ...
     pdfrand = -sqrt(3.)+2*sqrt(3.)*ran2(0)
     dxipart = col%cxfix*dt + pdfrand*col%cxran*sqrt(dt)
  ...
  if(scal%colflag.eq.1) then ! FLAG TELLING IF THIS PROCESS MUST BE CONSIDERED
     ...
     ! -------------------------------------------------
     pdfrand = -sqrt(3.)+2*sqrt(3.)*ran2(0)
     col%elde    = col%cefixe*dt + pdfrand*col%cerane*sqrt(dt)

     pdfrand = -sqrt(3.)+2*sqrt(3.)*ran2(0)
     col%ionde   = col%cefixi*dt + pdfrand*col%cerani*sqrt(dt)
  ...
  if(scal%one_orbit.eq.1) then
     out1d%elpower(bigstep)  = out1d%elpower(bigstep)  + col%cefixe*dt
     out1d%ionpower(bigstep) = out1d%ionpower(bigstep) + col%cefixi*dt
  endif
